...
namespace DefaultEcs
{
	...
	public readonly struct Entity : IDisposable, IEquatable<Entity>
	{
		...
		/// <summary>
		///		Read Only access to the component of type <typeparamref name="T"/> on the current <see cref="Entity"/>.
		/// </summary>
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public bool TryGet<T>(out T component)
		{
			ComponentPool<T> pool = ComponentManager<T>.Get(WorldId);
			if (pool != null && pool.TryGet(EntityId, out component))
				return true;
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public int GetIndex<T>() => ComponentManager<T>.Pools[WorldId].GetIndex(EntityId);
		...
		public Entity[] CopyTo(World world, int count) => CopyTo(world, count, ComponentCloner.Instance);

		public Entity[] CopyTo(World world, int count, ComponentCloner cloner)
		{
			world.CheckArgumentNullException(nameof(world));
			cloner.CheckArgumentNullException(nameof(cloner));
			...
			var copies = new Entity[count];
			for (int i = 0; i < count; i++)
				copies[i] = world.CreateEntity();
		...
		public void ReadAllComponents(IComponentReader reader) => Publisher.Publish(WorldId,
			new ComponentReadMessage(EntityId, reader.CheckArgumentNullException(nameof(reader))));
