    ...
    public abstract class AEntitySetSystem<T> : ISystem<T>
    {
        ...
        private class Runnable : IParallelRunnable
        {
            ...
            public void Run(int index, int maxIndex)
            {
                ...
                _system.Update(CurrentState,
                    _system.Set.GetEntities(start, index == maxIndex ? _system.Set.Count - start : EntitiesPerIndex));
            }
        }
        ...
        protected AEntitySetSystem(World world, Func<object, World, EntitySet> factory, IParallelRunner runner,
            int minEntityCountByRunnerIndex)
            : this(
                world is null ? throw new ArgumentNullException(nameof(world)) :
                factory is null ? throw new ArgumentNullException(nameof(factory)) : o => factory(o, world), runner,
        ...
        /// <exception cref="ArgumentNullException"><paramref name="world"/> is null.</exception>
        /// <exception cref="ArgumentNullException"><paramref name="factory"/> is null.</exception>
        protected AEntitySetSystem(World world, Func<object, World, EntitySet> factory, bool useBuffer, bool runWhenEmpty)
        ...
        /// <param name="useBuffer">Whether the entities should be copied before being processed.</param>
        /// <exception cref="ArgumentNullException"><paramref name="world"/> is null.</exception>
        protected AEntitySetSystem(World world, bool useBuffer = false, bool runWhenEmpty = false)
            : this(world, DefaultFactory, useBuffer, runWhenEmpty)
        {
        }
        ...
        {
            if (IsEnabled && (Set.Count > 0 || _runWhenEmpty))
            {
                PreUpdate(state);
