...
{
	public static class EntityRuleBuilderFactory
	{
		#region Fields
		...
		{
			_withPredicate = typeof(EntityQueryBuilder).GetTypeInfo().GetDeclaredMethods(nameof(EntityQueryBuilder.With))
		...
		private static Func<object, World, EntityQueryBuilder> GetEntityRuleBuilderFactory(Type type)
		{
			...
			while (type != null)
			{
				foreach (MethodInfo method in type.GetTypeInfo().DeclaredMethods
					         .Where(m => m.GetCustomAttribute<WithPredicateAttribute>(false) != null))
				{
					    ...
					    || method.ReturnType != typeof(bool))
					{
						throw new NotSupportedException(
							$"Can't apply {nameof(WithPredicateAttribute)} to \"{method.Name}\": method is not of type {nameof(ComponentPredicate<object>)}.");
					}
		...
		public static Func<object, World, EntityQueryBuilder> Create(Type type) =>
			_entityRuleBuilderFactories.GetOrAdd(type, GetEntityRuleBuilderFactory);
